...
for t in (-1.0, -0.5, 0.0, 0.5, 1.0):
    ...
    yaw = np.degrees(np.arctan2(2 * (q[3]*q[0] + q[1]*q[2]),
                                1 - 2*(q[1]**2 + q[2]**2)))
    yaw0 = np.degrees(np.arctan2(2 * (q0[3]*q0[0] + q0[1]*q0[2]),
                                 1 - 2*(q0[1]**2 + q0[2]**2)))
    dy = wrap(yaw - yaw0)
    dx = e.data.xpos[e.trunk_id][0] - x0
    mode = f"{dy/10:+5.1f}deg/s" if abs(dy) > 8 else "straight"
    print(f"turn {t:+4.1f} | 10s {mode} | dx {dx:+.3f} m  yaw {dy:+.1f}deg  z {e.data.xpos[e.trunk_id][2]:.3f}")
