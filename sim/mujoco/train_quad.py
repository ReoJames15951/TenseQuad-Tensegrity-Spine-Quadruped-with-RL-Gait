...
def evaluate(agent, n_envs=3, T=8.0, seed=99, render=False,
             stride_max=0.05, speed_target=0.15, gait_dir=-1.0,
             stride_min=0.0, heading_cmd=0.0, turn=0.0):
    """Run deterministic episodes, return avg metrics + per-env final x."""
    envs = gait_env.VecQuadGait(n=n_envs, seed=seed, dr=False,
    ...
    if render and viewer is not None:
        viewer.close()
    speeds = np.array(speeds); zs = np.array(zs)
...
def main():
    ...
    ap.add_argument("--stride-max", type=float, default=0.05,
                    help="max stride stroke (0 = stand-balance curriculum)")
    ap.add_argument("--speed-target", type=float, default=0.15,
                    help="walk speed reward target (m/s)")
    ap.add_argument("--gait-dir", type=float, default=-1.0,
                    help="stance-sweep sense; must match the ratchet-pad sign")
    ap.add_argument("--stride-min", type=float, default=0.0,
                    help="warm-up stride floor: force stride while the ratchet gait entrains")
    ap.add_argument("--heading-cmd", type=float, default=0.0,
    ...
    chk = ppo.Agent(gait_env.OBS_DIM, 8, hidden=args.hidden, lr=args.lr,
                    seed=args.seed)
    ...
    if args.eval_only:
        res = evaluate(chk, T=8.0, render=args.render,
                       ...
                       turn=args.turn)
        print(f"[eval] vx={res['vx']:+.3f} m/s  z={res['z']:.3f} m "
              f"(min {res['z_min']:.3f})  pitch_max={res['pitch_max']:.1f} deg  "
              f"dist={res['dist']:+.3f} m  yaw={res['yaw_drift']:+.1f}deg")
        return

    envs = gait_env.VecQuadGait(n=args.n_envs, seed=args.seed, dr=bool(args.dr),
                                stride_max=args.stride_max,
                                speed_target=args.speed_target,
                                gait_dir=args.gait_dir,
                                stride_min=args.stride_min)
    ...
    for it in range(args.iters):
        ...
        for t in range(H):
            ...
            buf_act[t] = a; buf_logp[t] = logp; buf_val[t] = v
            buf_rew[t] = rew; buf_done[t] = done; buf_trunc[t] = trunc
            obs = obs2.copy()
        buf_obs[H] = obs
        ...
        chk.train(buf_obs[:-1].reshape(-1, n_dims),
                  buf_act.reshape(-1, 8),
                  rets.reshape(-1), advs.reshape(-1),
                  buf_logp.reshape(-1))
        ...
        if (it + 1) % args.eval_every == 0:
            res = evaluate(chk, T=6.0, stride_max=args.stride_max,
                           speed_target=args.speed_target,
                           gait_dir=args.gait_dir,
                           stride_min=args.stride_min)
    ...
    res = evaluate(chk, T=8.0, stride_max=args.stride_max,
                   speed_target=args.speed_target, gait_dir=args.gait_dir,
                   stride_min=args.stride_min, heading_cmd=args.heading_cmd,
                   turn=args.turn)
    print(f"[final eval] vx={res['vx']:+.3f} m/s  z={res['z']:.3f} m "
          f"(min {res['z_min']:.3f})  pitch_max={res['pitch_max']:.1f} deg  "
          f"dist={res['dist']:+.3f} m  yaw={res['yaw_drift']:+.1f}deg")
